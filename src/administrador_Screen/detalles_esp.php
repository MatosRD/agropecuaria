...
if(!empty(['ticket'])){
    if(!empty($_POST['country'])){
        ...
        $estado = mysqli_query($conexion,"SELECT * FROM espera WHERE nopedido = '$id' ");
        while($fila = mysqli_fetch_assoc($estado)){
            if($ja == 'activo'){
                $sa = mysqli_query($conexion,"UPDATE espera SET estado = '$ja' WHERE nopedido = '$id' ");   
                $ff = mysqli_query($conexion," INSERT INTO pedidos_c SELECT * FROM espera");
                $f = mysqli_query($conexion," DELETE FROM espera WHERE nopedido = '$id'");
        ...
        <h1>Destalles</h1>
            ...
            <label for="country">
                <select id="country" name="country">
                    <?php
                        $consulta = mysqli_query($conexion,"SELECT * FROM espera WHERE nopedido = '$id' ");
...
<div class="tabl" >
    <?php  
    $suma = mysqli_query($conexion,"SELECT SUM(sub_total) FROM espera WHERE nopedido = '$id'");  
    $row = mysqli_fetch_row($suma);
    $sumat = $row[0];

    $descuento = mysqli_query($conexion,"SELECT SUM(descuento) FROM espera WHERE nopedido = '$id'");  
    ...
    ?>
        ...
        <table style="width: 80%;" >
            ...
            <tbody>
                <?php            
                    $consulta = "SELECT * FROM espera WHERE nopedido = '$id'";
